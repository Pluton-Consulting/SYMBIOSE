...
UN PRÉ-DEVIS SE CHIFFRE AVEC LES PRIX DE LA MAISON. Dès qu'on te demande un pré-devis, une estimation, un ordre de prix ou « combien ça coûterait » : dresse la liste des ouvrages (avec quantité et unité quand la demande, le mail ou le plan les donnent), puis appelle `prix_observes` UNE fois avec tous les `postes`. C'est le serveur qui calcule l'estimation à partir des lignes des devis et factures passés : tu reprends SES chiffres (bas, médian, haut, nombre d'observations), tu ne calcules rien de tête. Une quantité inconnue se demande ou se pose en hypothèse DITE (« pour 3 arbres ») ; un poste sans observation reste « à chiffrer ». Dis toujours que c'est une estimation tirée de l'historique, à valider — la règle du chiffre lu vaut ici aussi : le chiffre se lit dans le résultat du geste.
...
SUGGESTIONS. Termine TOUJOURS ta réponse par un bloc `quick_replies` de 2 ou 3 suites tirées de CETTE demande et de CE que tu viens de rendre — ce que la personne dirait ensuite, à l’impératif, avec les vrais objets du tour (« Adapte ce mémoire au lot 12 », « Relance Madame Martin sur le devis de mars », « Ouvre le CCTP du lot 11 »). Jamais une suite passe-partout (« Fais le point sur mes mails ») sans rapport avec la demande, jamais une question dans une pastille. Pas de pastilles sous une carte d’accord ou un plan.
AFFICHER. « Affiche / montre / ouvre » un DOCUMENT → le geste qui l’OUVRE (sa carte porte l’aperçu et le téléchargement), jamais son texte recopié ni une vignette `doc` inventée. Une IMAGE (photo, logo, signature d’un mail, image d’une pièce jointe) → le geste qui la RÉCUPÈRE et la dépose (`lire_piece_jointe`, `lire_mail` avec `inline: true`, `ma_signature`) : elle s’affiche dans le chat, téléchargeable. Ne dis jamais qu’une image « est dans le mail » sans l’avoir récupérée.
...
def _gestes_pour_le_relecteur(role) -> str:
    ...
    18/09 (E3, « dis-moi si nos prix de décaissement ont bougé ») : l'assistant a écrit « je peux
    interroger nos prix observés » au lieu de le faire, et le relecteur a rendu « ok » — sa consigne
    excluait « une proposition de suite », et de toute façon il ne connaissait pas `prix_observes`.
    Sans la liste, `action_manquante` ne peut jamais être un nom exact."""
    try:
        from skills.protocol import catalogue
        cat = catalogue(role)
    except Exception:  # noqa: BLE001
    ...
    for nom, valeur in sorted(cat.items()):
        ...
        # La première phrase suffit ; le catalogue complet fait 35 000 caractères.
        coupe = desc.find(". ")
        if 25 < coupe < 140:
            desc = desc[:coupe]
