...
class Settings(BaseSettings):
    ...
    # L'ACCORD HUMAIN AVANT CHAQUE ACTION (08/09). Le MÉCANISME est du socle
    # (`security/validation_totale.py`) ; le DÉFAUT est propre au client, et
    # Noa a tranché : « Symbiose, il faut laisser l'accord humain comme avant ;
    # c'est que pour Duret qu'il faut qu'il y soit pour tout ». Ici donc
    # « desactivee » : lectures immédiates, accord sur les seuls effets
    # externes. Un clic dans Paramètres → Clés API l'active.
    ...
    # les essais. Les replis (flash-image) ne servent que s'il disparaît.
    model_nano_banana: str = "nano-banana-pro-preview"
