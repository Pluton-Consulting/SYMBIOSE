...
def _substituer(gabarit: str, dest: dict) -> str:
    def _valeur(m):
        ...
        for k, v in dest.items():
            if str(k).strip().lower() == cle and v not in (None, ""):
                return str(v)
        return MANQUANT
...
def _normaliser(destinataires) -> list[dict]:
    """Accepte une liste de dicts, d'adresses nues, ou un mélange."""
    sortie: list[dict] = []
    for d in destinataires or []:
        if isinstance(d, dict):
            email = str(d.get("email") or d.get("mail") or d.get("adresse")
                        or d.get("de") or "").strip()
            entree = dict(d)
            entree["email"] = email
