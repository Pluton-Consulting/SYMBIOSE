...
export function Checkbox({ label = "Inclure la TVA", defaultChecked = true }: { label?: string; defaultChecked?: boolean }) {
  ...
  return (
    <label style={{ display: "inline-flex", alignItems: "center", gap: 10, cursor: "pointer", fontSize: 14, color: "var(--marque-text-body)", userSelect: "none" }}>
      <span onClick={() => setOn((v) => !v)} style={{
        ...
        border: `2px solid ${on ? "var(--marque-primary)" : "var(--marque-border)"}`,
      }}>{on ? "✓" : ""}</span>
      {label}
    </label>
  )
