    ...
    <div style={{ maxWidth: 320, paddingLeft: 4 }}>
      {steps.map((s, i) => (
        <div key={i} style={{ display: "grid", gridTemplateColumns: "20px 1fr", gap: 12, position: "relative", paddingBottom: i < steps.length - 1 ? 20 : 0 }}>
          {i < steps.length - 1 && <span style={{ position: "absolute", left: 9, top: 18, bottom: 0, width: 2, background: s.done ? "var(--color-primary-mid)" : "var(--color-border)" }} />}
          <span style={{ width: 20, height: 20, borderRadius: "50%", zIndex: 1, display: "grid", placeItems: "center", fontSize: 10, fontWeight: 800, color: "#fff",
            background: s.done ? "var(--color-primary)" : "var(--color-surface)", border: `2px solid ${s.done ? "var(--color-primary)" : "var(--color-border)"}` }}>{s.done ? "✓" : ""}</span>
          <div>
            <div style={{ fontSize: 13.5, fontWeight: 600, color: s.done ? "var(--color-text-primary)" : "var(--color-text-muted)" }}>{s.title}</div>
            <div style={{ fontSize: 11.5, color: "var(--color-text-muted)" }}>{s.time}</div>
          </div>
        </div>
      ))}
    </div>
